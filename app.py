...
OUT_OF_SCOPE = (
    "I can only answer questions that are covered in Module 1. Ask about prior "
    "authorization workflow, denial classification, documentation, or CoverMyMeds."
)
WELCOME_MESSAGE = (
    "Ask a Module 1 question about prior authorization workflow, denials, "
    "documentation, or CoverMyMeds."
)
...
STOPWORDS = {
    ...
    "summarize", "summary", "tell", "the", "there", "to", "what", "workflow",
}
...
def main() -> None:
    ...
    st.title("PA Coach")
    st.write(
        "Ask questions about Prior Authorization Foundations and the "
        "CoverMyMeds workflow. This assistant uses only the loaded Module 1 PDF."
    )
    st.caption(
    ...
    user_query = st.chat_input("Ask a Module 1 training question")
    if not user_query:
        return
